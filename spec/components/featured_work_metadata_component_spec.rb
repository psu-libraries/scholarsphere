...
RSpec.describe FeaturedWorkMetadataComponent, type: :component do
  ...
  describe 'rendering' do
    ...
    it 'renders creators' do
      expect(result.css('h5.creator_aliases').text).to eq 'Creator'
      expect(result.css('p.creator_aliases').text).to eq work_version.creator_aliases.first.alias
    end

    it 'renders keywords' do
      expect(result.css('h5.keyword').text).to eq 'Keywords'
      expect(result.css('p.keyword').text).to eq work_version.keyword.first
    end
  end
end
