...
RSpec.describe PdfRemediation::BuildAutoRemediatedWorkVersionJob do
  ...
  describe '#perform' do
    before do
      allow(PdfRemediation::BuildAutoRemediatedWorkVersion).to receive(:call)
    end
    ...
    context 'when the file resource with the given remediation_job_uuid exists' do
      it 'calls the service when perform_now is used' do
        ...
        expect(PdfRemediation::BuildAutoRemediatedWorkVersion)
          .to have_received(:call).with(file_resource, remediated_file_url)
      end
    end
  end
end
